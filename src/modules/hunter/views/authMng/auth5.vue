        ...
        <div class="content">
            <div class="main">
                <div style="margin: 2px;">
                    ...
                    <div>
                        <el-button type="primary"  icon="el-icon-save"  @click="saveMemberAccount">保存</el-button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    export default{
        ...
        methods:{
            ...
            saveMemberAccount(){
                if(!this.checkMemberChooseSingle()){
                    return;
                }
                var _this=this;
                var memberId="";
                ...
                $.each(this.$refs.memberTree.getCheckedNodes(), function (index, item) {
                    memberId=item.id;
                });
                $.each(this.$refs.accountTree.getCheckedNodes(), function (index, item) {
                    accountIds.push(item.id);
                });
                authAddMemberAccounts({memberId:memberId,accountIds:accountIds}).then(res=>{
