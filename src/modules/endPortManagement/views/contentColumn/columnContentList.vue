...
Vue.use(axios);
import {getTerminalList,getListColContent} from '@/api/api'
...
export default {
  ...
  methods: {
   ...
   //状态修改
    ...
    //编辑内容
    editContent(row){
     this.$router.push({path:'/base/editContent',query:{id:row.id}});
    },
    currentChange(num){
      this.curPage = num;
      this.getList();
