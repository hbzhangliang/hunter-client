    ...
    <div class="create-column" >
         ...
         <el-form-item label="栏目描述"  prop="desc">
          ...
          <el-form-item label="栏目数据类型"  prop="type"> 
                <el-radio-group  @change="getType" v-model.trim="columnData.type">
...
export default {
 ...
 created() {
    let colId  = this.$route.query.id;
    this.getDetail(colId);
  ...
  methods:{
    ...
    getDetail(id){
      ...
      axios.post('/api/column-manager/getSingleCol', param, {    
      }).then(res =>{
    ...
    updateColumn(){
      ...
      if (valid) {
             ...
             axios.post('/api/column-manager/updateCol', param, {    
              }).then(res =>{
                let code = res.data.code;
                if(code == -4001){//-4001栏目名称已经存在！
                        this.$message.error(res.data.message);
