<template>
  <section class="end-container">
    <NavBar></NavBar>
    <div class="content" style="height: 1060px;">
      <p class="main">
        <div class="banner">
        </div>
...
<script>
  ...
  import { bizDataSts } from '@/api/api'
  import $ from 'jquery'
  ...
  export default {
    ...
    methods:{
      getBizData(){
        ...
        console.log(paras);
        bizDataSts(paras).then(res => {
            if(null==res){
              this.$message('未查询到数据')
            }
            else {
              _this.columnOption.series=res.data;
              _this.columnOption.xAxis.categories=res.bizNames;
